...
const importantInformation = {
  addImportantInfomation: function (
    title,
    subtitle,
    description,
    image,
  ...
  getImportantInfomation: function (callback) {
    return query(
      `SELECT infoid, title, subtitle, description FROM importantinfo`
    ).then((result, err) => {
      if (err) {
        callback(err, null);
        return;
  ...
  getImportantInformationById: function (infoid, callback) {
    return query("SELECT * FROM importantinfo WHERE infoid = $1", [
      infoid,
    ]).then((result, err) => {
      if (err) {
        callback(err, null);
  ...
  updateImportantInformation: function (
    infoid,
    title,
    subtitle,
    description,
